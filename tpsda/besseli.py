...
from scipy.special import gammaln, logsumexp, i0e, i1e
...
def ive(nu,k):
    if np.isscalar(nu): 
        if nu==0: 
            return i0e(k)    # i0e(x) is much faster than scipy_ive(0,x)
        if nu==1: 
            return i1e(k)    # same here
    return scipy_ive(nu,k)
...
def log2sinh(x):
    """
    log [exp(x) - exp(-x)], implemented to avoid overflow for x > 0
    """
...
class LogBesselI:
    ...
    The degree, nu is stored in the callable, while k is supplied to the call,
    e.g.:
    ...
    def __init__(self, nu, n=5):
        """
        nu: Bessel-I order, with nu real, >= 0
            Although Bessel-I is defined for nu < 0, this code excludes that.
            
        n: the number of seres expansion terms used to patch underflow
           optional, we found the default 5 to work best 
        """
    ...
    def logCvmf(self, k = None, logk = None, exp_scale = False):
        ...
        Notes:
            ...
            logCvmf omits the term: -dim/2 log(2pi). 
        ...
        """
        nu = self.nu
        if nu==0.5: return logCvmf3(k, logk, exp_scale)  # faster: no Bessel
        ...
        kpos = k[pos]
        logkpos = logk[pos]
        logI = self(kpos, logkpos, exp_scale)
        y[pos] = nu*logkpos - logI
        return y
