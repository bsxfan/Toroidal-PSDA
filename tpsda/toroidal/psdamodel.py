...
from tpsda.subsphere.pca import randStiefel, lengthnorm, retract

from tpsda.vmf_onedim import gvmf, logNormConst

from tpsda.vmf_map import map_estimate as vmf_map_estimate
from tpsda.vmf_map import ml_estimate as vmf_ml_estimate
    ...
    def embed(self,Z):
        ...
        input: 
            ...
            Z: (t,T) horizontally stacked hidden variables, t of them. The sum of
                 hidden variable dimensions is T.
...
class Prior:
    ...
    def __init__(self,gamma,v):
        ...
        self.gamma_v = np.hstack([vmfi.kmu.ravel() for vmfi in self.vmf])
...
class ToroidalPSDA:
    ...
    def scoring(self,fast=False):
        return Scoring(self,fast)        
...
class Scoring:
    def __init__(self, model: ToroidalPSDA, fast = False, shortcut = False):
        ...
        if not uniform:
            nvec = [vmfi.kmu.ravel() for vmfi in zprior.vmf]
